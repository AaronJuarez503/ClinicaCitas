...
public class DatabaseWebSecurity {
    ...
    public UserDetailsManager customUsers(DataSource dataSource){
        ...
        users.setUsersByUsernameQuery("select login, clave, status from usuarios2 where login = ?");
        users.setAuthoritiesByUsernameQuery("select u.login, r.nombre from usuario_rol ur " +
                "inner join usuarios2 u on u.id = ur.usuario_id " +
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        http.authorizeHttpRequests(authorize -> authorize
                ...
                .requestMatchers("/usuarios/create", "/usuarios/save").permitAll()

                // Asignar permisos a URLs por ROLES
                .requestMatchers("/clinica/**").hasAnyAuthority("medico", "paciente")
                .requestMatchers("/medico/**").hasAnyAuthority("medico", "paciente")
